...
func _ready() -> void:
	...
	$Size.play("Default") #denna animationplayer ska spela effekten då powerupen plockas upp, men nu gör den ingenting
...
func _on_SpeedPowerUp_body_entered(body: Node) -> void:
	if body.is_in_group("Player"): #då spelaren plockar upp powerupen spelas ett ljud, hp går upp och en effekt spelas. Då effekten är klar tas scenen bort
		...
		player.power_up("speed") #anropar en funktion i Adventurer-scriptet som gör att spelaren får en speed powerup
